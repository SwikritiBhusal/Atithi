...
import React, { useState, useEffect } from 'react';
import { useLocation, useNavigate } from 'react-router-dom';
import { ArrowLeft, Lock, CheckCircle, AlertCircle, Loader } from 'lucide-react';
import Navbar from '../components/Navbar';
import './Payment.css';
...
export default function KhaltiPayment() {
  ...
  const initiatePayment = async () => {
    ...
    try {
      ...
      if (result.success) {
        // Store booking data for after payment redirect
        sessionStorage.setItem('pendingBooking', JSON.stringify({
          bookingId,
          homestay,
          bookingData,
        ...
        if (result.error) {
          console.error('Khalti Error Details:', result.error);
        }
      }
  ...
  const handleBack = () => {
    navigate(`/homestay/${homestay._id}/confirm`, {
      state: { homestay, bookingData, totalPrice, nights }
    });
  };
    ...
    <>
      ...
      <div className="khalti-payment-page">
        <div className="kp-container">
          ...
          <div className="kp-header">
            <div className="kp-lock-icon"><Lock size={32} /></div>
            <h1 className="kp-title">Secure Payment</h1>
            <p className="kp-subtitle">Complete your booking payment via Khalti</p>
          </div>
